...
void SetCharges(std::vector<double> xyz, std::vector<double> &charges, std::string mon_id, size_t n_mon, size_t nsites,
    ...
    // Alkali metal ions
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            ...
            for (size_t nv = 0; nv < n_mon; nv++) {
                charges[fst_ind + nv*nsites + 0] = -0.8726 * CHARGECON;
                charges[fst_ind + nv*nsites + 1] = 0.2909 * CHARGECON;
                charges[fst_ind + nv*nsites + 2] = 0.2909 * CHARGECON;
                charges[fst_ind + nv*nsites + 3] = 0.2909 * CHARGECON;
            }
        // END SECTION CHARGES
...
void SetPolfac(std::vector<double> &polfac, std::string mon_id, size_t n_mon, size_t nsites, size_t fst_ind) {
    ...
    if (mon_id == "li") {  // Lithium
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            ...
            for (size_t nv = 0; nv < n_mon; nv++) {
                polfac[fst_ind + nv*nsites + 0] = 0.9566;
                polfac[fst_ind + nv*nsites + 1] = 0.3632;
                polfac[fst_ind + nv*nsites + 2] = 0.3632;
                polfac[fst_ind + nv*nsites + 3] = 0.3632;
            }
        // =====>> END SECTION POLFACS <<=====
...
void SetPol(std::vector<double> &pol, std::string mon_id, size_t n_mon, size_t nsites, size_t fst_ind) {
    ...
    if (mon_id == "li") {  // Lithium
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            ...
            for (size_t nv = 0; nv < n_mon; nv++) {
                pol[fst_ind + nv*nsites + 0] = 0.9566;
                pol[fst_ind + nv*nsites + 1] = 0.3632;
                pol[fst_ind + nv*nsites + 2] = 0.3632;
                pol[fst_ind + nv*nsites + 3] = 0.3632;
            }
        // =====>> END SECTION POLS <<=====

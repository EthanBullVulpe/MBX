...
void SetCharges(std::vector<double> xyz, std::vector<double> &charges, std::string mon_id, size_t n_mon, size_t nsites,
    ...
    // Alkali metal ions
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            ...
            for (size_t nv = 0; nv < n_mon; nv++) {
                charges[fst_ind + nv*nsites + 0] = -0.8748 * CHARGECON;
                charges[fst_ind + nv*nsites + 1] = 0.2916 * CHARGECON;
                charges[fst_ind + nv*nsites + 2] = 0.2916 * CHARGECON;
                charges[fst_ind + nv*nsites + 3] = 0.2916 * CHARGECON;
            }
        // END SECTION CHARGES
...
void SetPolfac(std::vector<double> &polfac, std::string mon_id, size_t n_mon, size_t nsites, size_t fst_ind) {
    ...
    if (mon_id == "li") {  // Lithium
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            ...
            for (size_t nv = 0; nv < n_mon; nv++) {
                polfac[fst_ind + nv*nsites + 0] = 0.8383;
                polfac[fst_ind + nv*nsites + 1] = 0.2763;
                polfac[fst_ind + nv*nsites + 2] = 0.2763;
                polfac[fst_ind + nv*nsites + 3] = 0.2763;
            }
        // =====>> END SECTION POLFACS <<=====
...
void SetPol(std::vector<double> &pol, std::string mon_id, size_t n_mon, size_t nsites, size_t fst_ind) {
    ...
    if (mon_id == "li") {  // Lithium
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            ...
            for (size_t nv = 0; nv < n_mon; nv++) {
                pol[fst_ind + nv*nsites + 0] = 0.8383;
                pol[fst_ind + nv*nsites + 1] = 0.2763;
                pol[fst_ind + nv*nsites + 2] = 0.2763;
                pol[fst_ind + nv*nsites + 3] = 0.2763;
            }
        // =====>> END SECTION POLS <<=====
...
void SetC6LongRange(std::vector<double> &c6_lr, std::string mon_id, size_t n_mon, size_t natoms, size_t fst_ind) {
    ...
    if (mon_id == "f") {  // Fluoride
        ...
        for (size_t nv = 0; nv < n_mon; nv++) { 
            c6_lr[nv * natoms + fst_ind] = 13.990035739768501; // A
            c6_lr[nv * natoms + fst_ind] = 5.250942772493335; // B
            c6_lr[nv * natoms + fst_ind] = 5.250942772493335; // B
            c6_lr[nv * natoms + fst_ind] = 5.250942772493335; // B
        }
